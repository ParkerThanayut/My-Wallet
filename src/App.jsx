...
import { 
  ...
  onSnapshot, updateDoc, arrayUnion, setDoc, getDoc, enableIndexedDbPersistence 
} from 'firebase/firestore';
import { getStorage, ref, uploadBytes, getDownloadURL } from 'firebase/storage';

// ------------------------------------------------------------------
// 🔑 การตั้งค่า Firebase (รหัสของคุณ)
// ------------------------------------------------------------------
...
const App = () => {
  ...
  useEffect(() => {
    ...
    // FIX: ใส่ try-catch กัน Error "Offline"
    const loadCategories = async () => {
      try {
        ...
        console.log("Using default categories (Offline/New user)");
        // ไม่ต้องทำอะไร ใช้ค่า Default ไปก่อน
      }
    };
    loadCategories();
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-24 md:pb-0">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 pb-24 rounded-b-[2rem] shadow-lg text-white relative">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2"><button onClick={() => setShowCatManager(true)} className="bg-white/20 p-2 rounded-full hover:bg-white/30"><Settings size={20}/></button><button onClick={handleLogout} className="bg-white/20 p-2 rounded-full hover:bg-red-500/50 transition-colors"><LogOut size={20}/></button></div>
        </div>
